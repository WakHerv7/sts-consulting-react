...
function SelectInput({title, nameText, placeholderText, requiredValue, optionList}) {
  return (
    <div>    
    <label className="commonSelectInputLabel" htmlFor={nameText}>{title} <span>{requiredValue ? '*' : ''}</span> </label>
    
    <select className="commonSelectInput" name={nameText} id={nameText} required={requiredValue}>
